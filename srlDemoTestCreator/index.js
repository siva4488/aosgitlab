...
const baseUrl = 'https://stormrunner-load.saas.hpe.com/v1/', // 'https://ptalb002qtso.saas.hpe.com/v1/',
    ...
    modifiedScriptFilePath = path.dirname(cliArgs.scriptPath) + '/temp/' + path.basename(cliArgs.scriptPath),
    tempModifiedFolder = modifiedScriptFilePath.substring(0, modifiedScriptFilePath.lastIndexOf('.'));

function validateInput(cb) {
    console.log('validateInput - start');
    if (!cliArgs.scriptPath) {
        return cb('Error: You must provide script path');
    }
    if (!cliArgs.tenantId) {
...
function replaceAutParams(cb) {
    ...
    let origZip = new Zip(cliArgs.scriptPath);
    origZip.extractAllTo(tempModifiedFolder, true);
    fs.writeFile(tempModifiedFolder + '/AOS_URL.dat', ipData);

    let zip = new FolderZip();
    zip.zipFolder(tempModifiedFolder, { excludeParentFolder: true }, () => {
